// Barbershop with array of LEDs for waiting room
/*
#include "mbed.h"
...
DigitalOut waiting_led_vect[8] = {WAITING_LED1, WAITING_LED2, WAITING_LED3, WAITING_LED4, WAITING_LED5, WAITING_LED6, WAITING_LED7, WAITING_LED8};
//Define Interrupts
...
//Functions
void waitingroom_led_func() {
    for (int i = 0; i < sizeof(waiting_led_vect) / sizeof(waiting_led_vect[0]); i++) {  //this is a new way of doing waiting_vectot.size() form the vector library.
        waiting_led_vect[i] = 0;
    }
    for (int j = 0; j < waitingroom_counter; j++) {
        waiting_led_vect[j] = 1;
    }
}
