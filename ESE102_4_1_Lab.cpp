/*
 ...
 //Final Project for Lab 4, with RTOS.
 //Selects & plays one of 8 tunes, with name displayed on LCD.
 ...
 InterruptIn play(D12);     //for Play button
...
//Waiting for Song
 ...
 void Tune_select(void const *args) {
     while(1) {
         if (primed) {              // "primed" is set by Interrupt
             switch(choose) {       //ready song selection and load song pointer
 ...
 void Play_tune(void const *args) {
     while(1) {
         if (playing){
             //resume PWM operation, stop after last song
             speaker.resume();
 ...
 void ISR() {           //responds to press of Play button, and sets "primed"
     if(playing == 0) { //only set "primed" of song not playing
         primed = 1;
     }
 }
